...
static v8::Handle<Value> AvailableFilters(const Arguments& args)
{
    ...
    VJXCoreImageFilter *filter = (VJXCoreImageFilter *)args.Holder()->GetPointerFromInternalField(0);
...
static v8::Handle<Value> SelectFilter(const Arguments& args)
{
    HandleScope handleScope;
    BOOL ret = NO;
    VJXCoreImageFilter *filterInstance = (VJXCoreImageFilter *)args.Holder()->GetPointerFromInternalField(0);
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    v8::Handle<Value> arg = args[0];
    v8::String::Utf8Value value(arg);
