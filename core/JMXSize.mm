...
+ (v8::Persistent<FunctionTemplate>)jsClassTemplate
{
    ...
    classTemplate->SetClassName(String::New("Size"));
...
- (v8::Handle<v8::Object>)jsObj
{
    ...
    v8::Handle<FunctionTemplate> classTemplate = [JMXSize jsClassTemplate];
...
v8::Handle<v8::Value> JMXSizeJSConstructor(const v8::Arguments& args)
{
    HandleScope handleScope;
    //v8::Locker locker;
    v8::Persistent<FunctionTemplate> classTemplate = [JMXSize jsClassTemplate];
    int width = 0;
    int height = 0;
    if (args.Length() >= 2) {
