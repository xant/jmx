...
#pragma mark JMXSpectralChannel

@interface JMXSpectralChannel : NSObject
{
    UInt32 bufLen;
    UInt32 fftLen;
    Float32 *inputBuf;
...
@implementation JMXSpectralChannel
...
#pragma mark JMXSpectrumAnalyzer

@interface JMXSpectrumAnalyzer (Private)
...
- (id)initWithSize:(UInt32)size hopSize:(UInt32)hop channels:(UInt32)channelsNum maxFrames:(UInt32)frames
{
    ...
    if (self) {
        ...
        for (i = 0; i < numChannels; i++) {
            JMXSpectralChannel *channel = [[JMXSpectralChannel alloc] initWithSize:(UInt32)fftSize frames:(UInt32)maxFrames];
...
- (UInt32)inputSize
{
    ...
    JMXSpectralChannel *channel = [channels objectAtIndex:0];
    if (channel)
...
- (void)doWindowing
{
	...
	for (UInt32 i = 0; i < numChannels; i++) {
		Float32 *x = (Float32 *)((JMXSpectralChannel *)[channels objectAtIndex:i]).fftBuf;
...
- (void)doInvFFT
{
	...
	for (UInt32 i = 0; i < numChannels; i++) 
	{
		...
		vDSP_ztoc(&dspSplitComplex[i], 1, (DSPComplex*)((JMXSpectralChannel *)[channels objectAtIndex:i]).fftBuf, 2, half);		
		float scale = 0.5 / fftSize;
		vDSP_vsmul(((JMXSpectralChannel *)[channels objectAtIndex:i]).fftBuf, 1, &scale, 
                   (void *)((JMXSpectralChannel *)[channels objectAtIndex:i]).fftBuf, 1, fftSize);
	}
	//printf("<-DoInvFFT %g %g\n", direction, mChannels[0].mFFTBuf()[0], mChannels[0].mFFTBuf()[200]);
}
